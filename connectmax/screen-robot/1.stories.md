...
A partir de imagem/frame ou dump, detecta ícones com bounds e metadados. Alimenta a composição da árvore DOM (US-17).

### US-14 — Extrair textos

Extrai textos da tela via OCR.

A partir de imagem/frame, reconhece strings e posições (bounds). Alimenta a composição da árvore DOM (US-17).

### US-15 — Extrair listas

Extrai listas com itens filhos.

Identifica nós de lista e seus itens na tela/dump, com hierarquia lista → filhos. Alimenta a composição da árvore DOM (US-17).

### US-16 — Extrair imagens

Extrai imagens/fotos da tela.

Detecta regiões de imagem/foto com bounds e metadados. Alimenta a composição da árvore DOM (US-17).

### US-17 — Extrair árvore DOM

Compõe a hierarquia navegável a partir dos elementos tipados.

Recebe ícones, textos, listas e imagens já extraídos (US-13..16) e monta a árvore raiz → filhos (estilo DOM). É a leitura estruturada completa do que está na tela.
