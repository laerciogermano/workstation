...
```gherkin
Cenário: US-13 Árvore DOM é composta em duas fases
  Dado uma imagem da tela (screenshot/frame) ou dump
  Quando primeiro apenas textos são extraídos
  E em seguida o restante é extraído e a hierarquia é composta
  Então a árvore DOM navegável é devolvida
```

### SC-17 — Extrair apenas textos (fase 1 da árvore)

```gherkin
Cenário: SC-17 Apenas textos da fase 1 são extraídos
  Dado uma imagem da tela (screenshot/frame) ou dump
  Quando a fase 1 extrai apenas textos
  Então textos tipados existem com string e bounds
```

### SC-18 — Extrair o restante e compor árvore DOM

```gherkin
Cenário: SC-18 Restante é extraído e árvore DOM é composta
  Dado textos da fase 1 e uma imagem da tela (screenshot/frame) ou dump
...
```gherkin
Cenário: US-14 Ícones tipados são extraídos
  Dado uma imagem da tela (screenshot/frame) ou dump
  Quando o reconhecimento de ícones é executado
  Então ícones tipados existem com bounds e metadados
```

### SC-19 — Extrair ícones da tela

```gherkin
Cenário: SC-19 Ícones da tela são extraídos
  Dado uma imagem da tela (screenshot/frame) ou dump
  Quando o reconhecimento de ícones é executado
  Então ícones tipados existem com bounds e metadados
```
...
```gherkin
Cenário: US-15 Textos tipados são extraídos
  Dado uma imagem da tela (screenshot/frame)
  Quando o OCR de textos é executado
  Então textos tipados existem com string e bounds
```

### SC-20 — Extrair textos (OCR)

```gherkin
Cenário: SC-20 Textos via OCR são extraídos
  Dado uma imagem da tela (screenshot/frame)
  Quando o OCR de textos é executado
  Então textos tipados existem com string e bounds
```
...
```gherkin
Cenário: US-16 Listas com itens filhos são extraídas
  Dado uma imagem da tela (screenshot/frame) ou dump
  Quando o reconhecimento de listas é executado
  Então listas tipadas existem com itens filhos
```

### SC-21 — Extrair listas com itens filhos

```gherkin
Cenário: SC-21 Listas com itens filhos são extraídas
  Dado uma imagem da tela (screenshot/frame) ou dump
  Quando o reconhecimento de listas é executado
  Então listas tipadas existem com itens filhos
```
...
```gherkin
Cenário: US-17 Imagens/fotos tipadas são extraídas
  Dado uma imagem da tela (screenshot/frame) ou dump
  Quando o reconhecimento de imagens/fotos é executado
  Então imagens tipadas existem com bounds e metadados
```

### SC-22 — Extrair imagens/fotos

```gherkin
Cenário: SC-22 Imagens/fotos são extraídas
  Dado uma imagem da tela (screenshot/frame) ou dump
  Quando o reconhecimento de imagens/fotos é executado
  Então imagens tipadas existem com bounds e metadados
```
...
### SC-23 — Salvar sessão em arquivo

```gherkin
Cenário: SC-23 Sessão é gravada
  Dado estado em memória e path da sessão
  Quando a serialização grava o arquivo
  Então o arquivo de sessão existe
```
...
### SC-24 — Remover sessão do disco

```gherkin
Cenário: SC-24 Sessão é removida do disco
  Dado um path de sessão
...
### SC-25 — Recuperar sessão do arquivo

```gherkin
Cenário: SC-25 Sessão é recuperada
  Dado um arquivo de sessão existente
  Quando a leitura reaplica o contexto
  Então o runtime possui o estado restaurado
```
