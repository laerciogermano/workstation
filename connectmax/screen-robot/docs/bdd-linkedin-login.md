...
Funcionalidade: Login no LinkedIn com agent Android via Node
  ...
  Cenário: Login a partir da home ou já na tela de login
    ...
    E os elementos e informações da tela são extraídos
    Então existe o botão de login (ex.: "Entrar" / "Sign in")
...
| Extrair elementos e informações | `E os elementos… são extraídos` → `extractElements()` |
