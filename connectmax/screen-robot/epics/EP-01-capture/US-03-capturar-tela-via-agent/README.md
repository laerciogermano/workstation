...
Como agente do screen-robot, quero **capturar a tela via agent no device** (APK / endpoint), para o mesmo Frame padronizado funcionar em hardware sem depender só de screencap ADB.

## Cenários

### SC-01 — Agent devolve frame

Dado agent acessível no endpoint configurado, quando **chamo a captura via agent**, então recebo **Frame** no mesmo schema da US-01.
