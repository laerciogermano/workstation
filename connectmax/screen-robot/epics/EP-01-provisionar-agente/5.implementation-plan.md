...
|---|---------|---------|----------|----------|--------|
| 1 | Dev → Provisioner: `provisionAgent(cfg)` | Entrada do épico: provisionar um agente usável | `cfg` (serial, kind?, timeout?, startScript?) | Recebe o cfg, valida campos mínimos e dispara as etapas SC-01, SC-02 e SC-03 em sequência | Promise de `AgentHandle` |
| 2 | Provisioner → Provisioner: resolve serial/timeout/kind | Normalizar config antes de falar com o device | `cfg` bruto + env (`ANDROID_SERIAL`) | Lê serial/kind/timeout de cfg, device e env; aplica defaults e monta ProvisionConfig interno | `ProvisionConfig` resolvido |
| 3 | Provisioner → RuntimeStarter: `start(cfg)` se não reachable | Garantir runtime Android no ar (SC-01) | `kind`, `startScript?`, `host?` | Consulta se o runtime já responde; se não, chama RuntimeStarter.start com o cfg | `reachable` ou erro |
| 4 | RuntimeStarter → Device: `scripts/start.sh` | Subir container redroid ou AVD | Script + args do kind | Executa o script de start (docker/AVD) no host para materializar o Android | pedido de start |
| 5 | Device → RuntimeStarter: `up (porta/processo)` | Confirmar que o runtime subiu | status do container/AVD | Device/host sinaliza processo e porta disponíveis | `up` |
| 6 | RuntimeStarter → Provisioner: `reachable` | Sinalizar que o host já responde | `up` do device | Devolve ao Provisioner que o runtime está alcançável e a etapa SC-01 pode seguir | `true` / reachable |
| 7 | Provisioner → AdbClient: `connectIfTcp(serial)` | TCP precisa de `adb connect` (SC-02) | `serial` host:port | Se o serial for TCP (host:port), prepara e dispara adb connect antes do wait | Conexão tentada |
| 8 | AdbClient → Device: `adb connect` | Estabelecer canal ADB TCP | `host:port` | Roda `adb connect` contra o host:port do serial e registra o resultado | Device listável ou falha |
| 9 | Provisioner → AdbClient: `wait-for-device` | Esperar estado `device` online | `serial`, timeout | Entra em loop chamando wait-for-device até o serial ficar `device` ou estourar o timeout | ok / retry |
| 10 | AdbClient → Device: `adb -s serial wait-for-device` | Bloquear até o adb ver o device | `serial` | Executa `adb -s <serial> wait-for-device` bloqueando até o adb enxergar o device | Serial `device` |
| 11 | Provisioner → AdbClient: `getprop sys.boot_completed` | Confirmar boot Android (SC-03) | `serial` | Em loop, solicita getprop de sys.boot_completed até ler `1` ou timeout | Valor da prop |
| 12 | AdbClient → Device: `adb shell getprop` | Ler propriedade no device | `sys.boot_completed` | Roda `adb shell getprop sys.boot_completed` no device e devolve a string | `"0"`\\|`"1"`\\|vazio |
| 13 | Device → AdbClient: `"1"` | Boot completo | — | Propaga o valor `1` indicando boot completo para o Provisioner | Booted |
| 14 | Provisioner → Dev: `AgentHandle` | Entregar handle pronto para EP-02..06 | Estado interno do provisionamento | Monta e retorna AgentHandle com serial, kind e provisionedAt ao caller | `{ serial, kind, provisionedAt }` |
...
| # | Chamada | Por quê | Entradas | Execução | Saídas |
|---|---------|---------|----------|----------|--------|
| 1 | Provisioner → RuntimeStarter: `isReachable(cfg)?` | Evitar subir runtime já disponível | `kind`, `serial`/`host` | Faz probe de porta/processo do runtime configurado e responde se já está up | `true`\\|`false` |
| 2 | RuntimeStarter → Provisioner: `true` | Atalho: nada a subir | — | Encerra a etapa sem spawn porque o Android já está alcançável | reachable imediato |
| 3 | RuntimeStarter → Host: `spawn startScript` | Subir Docker/Colima ou AVD | `startScript`, args | Dispara o processo do startScript no host (Docker/Colima ou launcher do AVD) | PID / processo |
| 4 | Host → Device: container/AVD up | Materializar o Android | Imagem/AVD configurado | Sobe o container redroid ou o emulador AVD até o processo estar rodando | Runtime em boot |
| 5 | RuntimeStarter → Device: poll porta/status | Saber quando fica alcançável | porta / health | Fica em poll curto lendo porta/status até o runtime responder | status up |
| 6 | RuntimeStarter → Provisioner: `reachable` | Fechar SC-01 | — | Retorna reachable ao Provisioner quando o probe passa | `reachable` |
...
| # | Chamada | Por quê | Entradas | Execução | Saídas |
|---|---------|---------|----------|----------|--------|
| 1 | Provisioner → AdbClient: `connectIfTcp(serial)` | TCP exige connect explícito | `serial` | Detecta serial TCP e, se for o caso, chama connect antes de esperar o device | Tentativa registrada |
| 2 | AdbClient → Device: `adb connect host:port` | Abrir sessão ADB | `host:port` | Executa `adb connect host:port` e verifica se a sessão TCP foi aceita | connected / failed |
| 3 | Provisioner → AdbClient: `wait-for-device` | Aguardar estado `device` | `serial`, timeout | Chama wait-for-device com o serial e o timeout restante do provisionamento | ok / erro |
| 4 | AdbClient → Device: `adb -s serial wait-for-device` | Sincronizar com o adb server | `serial` | Roda `adb -s <serial> wait-for-device` até o estado virar `device` | online |
| 5 | AdbClient → Provisioner: `ok` | Serial pronto | — | Sinaliza sucesso e sai do loop de retry | prossegue |
| 6 | AdbClient → Provisioner: `erro / retry` | Ainda não online | código/erro adb | Sinaliza falha transitória para o Provisioner tentar de novo | sinal de retry |
| 7 | Provisioner → AdbClient: `sleep(2000)` | Backoff antes do próximo poll | `2000ms` | Aguarda 2s (backoff) e reentra no loop de connect/wait | reentra no loop |
...
| # | Chamada | Por quê | Entradas | Execução | Saídas |
|---|---------|---------|----------|----------|--------|
| 1 | Provisioner → AdbClient: `getprop sys.boot_completed` | Única fonte de verdade do boot | `serial` | Periodicamente pede getprop sys.boot_completed até obter `1` ou timeout | prop lida |
| 2 | AdbClient → Device: `getprop` | Executar no shell do device | `sys.boot_completed` | Executa `adb shell getprop sys.boot_completed` e devolve a string bruta | string da prop |
| 3 | Device → AdbClient: `"0"`\\|`"1"`\\|vazio | Entrega o valor atual da prop (`0`, `1` ou vazio) ao AdbClient | — | Resposta shell | valor bruto |
| 4 | AdbClient → Provisioner: `Booted` | Boot concluído | `"1"` | Marca Booted e encerra o wait quando a prop for `1` | `{ boot: true }` |
| 5 | Provisioner → AdbClient: `sleep(2000)` | Aguardar e tentar de novo | `2000ms` | Espera 2s e repete o getprop enquanto o boot ainda não completou | reentra no loop |
