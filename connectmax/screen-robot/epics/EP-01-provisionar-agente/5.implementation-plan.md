...
|---|---------|---------|----------|----------|--------|
| 1 | Dev → Provisioner: `provisionAgent(cfg)` | Entrada do épico: provisionar um agente usável | `cfg` (serial, kind?, timeout?, startScript?) | Valida e orquestra SC-01→SC-03 | Promise de `AgentHandle` |
| 2 | Provisioner → Provisioner: resolve serial/timeout/kind | Normalizar config antes de falar com o device | `cfg` bruto + env (`ANDROID_SERIAL`) | Defaults e precedência de campos | `ProvisionConfig` resolvido |
| 3 | Provisioner → RuntimeStarter: `start(cfg)` se não reachable | Garantir runtime Android no ar (SC-01) | `kind`, `startScript?`, `host?` | Só sobe se `isReachable` for false | `reachable` ou erro |
| 4 | RuntimeStarter → Device: `scripts/start.sh` | Subir container redroid ou AVD | Script + args do kind | Spawn do processo/container | Processo/porta up |
| 5 | RuntimeStarter → Provisioner: `reachable` | Sinalizar que o host já responde | — | Retorno da etapa SC-01 | `true` / reachable |
| 6 | Provisioner → AdbClient: `connectIfTcp(serial)` | TCP precisa de `adb connect` (SC-02) | `serial` host:port | Detecta TCP e conecta | Conexão tentada |
| 7 | AdbClient → Device: `adb connect` | Estabelecer canal ADB TCP | `host:port` | Comando adb | Device listável ou falha |
| 8 | Provisioner → AdbClient: `wait-for-device` | Esperar estado `device` online | `serial`, timeout | Loop até online ou timeout | ok / retry |
| 9 | AdbClient → Device: `adb -s serial wait-for-device` | Bloquear até o adb ver o device | `serial` | Comando adb | Serial `device` |
| 10 | Provisioner → AdbClient: `getprop sys.boot_completed` | Confirmar boot Android (SC-03) | `serial` | Poll periódico | Valor da prop |
| 11 | AdbClient → Device: `adb shell getprop` | Ler propriedade no device | `sys.boot_completed` | Shell remoto | `"0"`\\|`"1"`\\|vazio |
| 12 | Device → AdbClient: `"1"` | Boot completo | — | Resposta do getprop | Booted |
| 13 | Provisioner → Dev: `AgentHandle` | Entregar handle pronto para EP-02..06 | Estado interno do provisionamento | Monta objeto de saída | `{ serial, kind, provisionedAt }` |
...
| # | Chamada | Por quê | Entradas | Execução | Saídas |
|---|---------|---------|----------|----------|--------|
| 1 | Provisioner → RuntimeStarter: `isReachable(cfg)?` | Evitar subir runtime já disponível | `kind`, `serial`/`host` | Probe de porta/processo | `true`\\|`false` |
| 2 | RuntimeStarter → Provisioner: `true` | Atalho: nada a subir | — | alt já alcançável | reachable imediato |
| 3 | RuntimeStarter → Host: `spawn startScript` | Subir Docker/Colima ou AVD | `startScript`, args | Spawn no host | PID / processo |
| 4 | Host → Device: container/AVD up | Materializar o Android | Imagem/AVD configurado | docker run / emulator | Runtime em boot |
| 5 | RuntimeStarter → Device: poll porta/status | Saber quando fica alcançável | porta / health | Loop curto de probe | status up |
| 6 | RuntimeStarter → Provisioner: `reachable` | Fechar SC-01 | — | Retorno final | `reachable` |
...
| # | Chamada | Por quê | Entradas | Execução | Saídas |
|---|---------|---------|----------|----------|--------|
| 1 | Provisioner → AdbClient: `connectIfTcp(serial)` | TCP exige connect explícito | `serial` | Se TCP → connect | Tentativa registrada |
| 2 | AdbClient → Device: `adb connect host:port` | Abrir sessão ADB | `host:port` | Comando adb | connected / failed |
| 3 | Provisioner → AdbClient: `wait-for-device` | Aguardar estado `device` | `serial`, timeout | Chamada wait-for-device | ok / erro |
| 4 | AdbClient → Device: `adb -s serial wait-for-device` | Sincronizar com o adb server | `serial` | Bloqueio até device | online |
| 5 | AdbClient → Provisioner: `ok` | Serial pronto | — | alt status device | prossegue |
| 6 | AdbClient → Provisioner: `erro / retry` | Ainda não online | código/erro adb | alt falha | sinal de retry |
| 7 | Provisioner → AdbClient: `sleep(2000)` | Backoff antes do próximo poll | `2000ms` | Espera | reentra no loop |
...
| # | Chamada | Por quê | Entradas | Execução | Saídas |
|---|---------|---------|----------|----------|--------|
| 1 | Provisioner → AdbClient: `getprop sys.boot_completed` | Única fonte de verdade do boot | `serial` | Poll até `1` ou timeout | prop lida |
| 2 | AdbClient → Device: `getprop` | Executar no shell do device | `sys.boot_completed` | `adb shell getprop` | string da prop |
| 3 | Device → AdbClient: `"0"`\\|`"1"`\\|vazio | Estado atual do boot | — | Resposta shell | valor bruto |
| 4 | AdbClient → Provisioner: `Booted` | Boot concluído | `"1"` | alt boot == 1 | `{ boot: true }` |
| 5 | Provisioner → AdbClient: `sleep(2000)` | Aguardar e tentar de novo | `2000ms` | else no alt | reentra no loop |
