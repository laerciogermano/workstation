...
|---|---------|---------|----------|----------|--------|
| 1 | Dev → Session: `saveSession(path, state)` | Persistir progresso do robot | `path`, `SessionState` | Serializa JSON | Promise `absPath` |
| 2 | Session → Session: merge `savedAt` | Carimbar momento do save | state atual | Acrescenta timestamp ISO | state enriquecido |
| 3 | Session → FS: `mkdir` + write JSON | Gravar no disco | `path`, JSON | Cria dirs + write atômico | arquivo escrito |
| 4 | FS → Session: `ok` | Persistência ok | — | Retorno FS | sucesso |
| 5 | Session → Dev: `absPath` | Path canônico | path resolvido | Resolve | string absoluta |
...
| # | Chamada | Por quê | Entradas | Execução | Saídas |
|---|---------|---------|----------|----------|--------|
| 1 | Dev → Session: `removeSession(path)` | Limpar sessão persistida | `path` | Unlink idempotente | Promise `{ removed }` |
| 2 | Session → FS: `unlink` se existe | Apagar arquivo | `path` | Ignore se já ausente | removed true/false |
| 3 | Session → Dev: limpar contexto runtime | Memória alinhada ao disco | handles/estado em RAM | Reset contexto | contexto limpo |
| 4 | Session → Dev: `{ removed: boolean }` | Resultado idempotente | — | Resolve | flag removed |
...
| # | Chamada | Por quê | Entradas | Execução | Saídas |
|---|---------|---------|----------|----------|--------|
| 1 | Dev → Session: `restoreSession(path)` | Retomar de um save | `path` | Read + validate | Promise `SessionState` |
| 2 | Session → FS: read JSON | Carregar bytes | `path` | fs.readFile | string/objeto bruto |
| 3 | Session → Dev: erro | Arquivo ausente/inválido | código | alt falha | `SESSION_NOT_FOUND`\\|`INVALID` |
| 4 | Session → Session: validar schema | Garantir campos usáveis | JSON parseado | Check campos obrigatórios | state válido |
| 5 | Session → Dev: `state` | Sessão restaurada | — | Resolve | `SessionState` |
