...
/** Escapa texto para `adb shell input text` (espaços = %s). */
...
async function runStep(device, step, i) {
  ...
  switch (action) {
    ...
    case "text": {
      ...
      // Preferência: clipboard + paste (melhor com acentos); fallback input text
      if (step.method === "input") {
        adb(device, ["shell", "input", "text", escapeInputText(text)]);
      } else {
        // cmd clipboard precisa de API recente; usa service call + keyevent paste quando possível
        // Mais portátil: input text escapado
        adb(device, ["shell", "input", "text", escapeInputText(text)]);
      }
      break;
    }
