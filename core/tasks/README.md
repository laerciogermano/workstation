...
Fluxo de desenvolvimento (IA): clone **na pasta `TSK-<nn>-<titulo>` em `tasks/`** → branch homônima → TDD → implementar → testar → PR → **Pendente aprovação** → **humano valida** → próximo robô. Ordem = Gantt das tasks; sequencial espera, paralelo pode subir PRs separados. Detalhe: [`processo/3.desenvolvimento`](../processo/3.desenvolvimento/README.md).
