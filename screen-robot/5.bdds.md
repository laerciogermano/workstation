...
**Piloto:** [`src/scripts/linkedin-login.js`](src/scripts/linkedin-login.js) — limpa screenshots → `resetInstance` → provision → LinkedIn → print tela inicial → `findByText("Sign in with Email")` → lista OCR no console (sem digitar credenciais / sem `saveSession`).  
