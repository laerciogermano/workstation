...
**US:** US-13..16 · US-23 · **SC:** SC-17..21 · SC-29..30 · **Cenários:** [`4.scenarios.md#ep-05--extrair-elementos`](4.scenarios.md#ep-05--extrair-elementos) · **Plano:** [`implementation-plan/EP-05-extrair-elementos.md`](implementation-plan/EP-05-extrair-elementos.md)

### US-13 — Extrair elementos (OCR)

```gherkin
Cenário: US-13 Lista de elementos é composta em duas fases
  Dado uma imagem/frame da tela (screenshot ADB, stream ou câmera)
  Quando OCR extrai textos + bounds e devolve a lista
  E em seguida visão acrescenta ícones/listas/imagens na mesma lista
  Então a lista completa de elementos é devolvida
```

#### SC-17 — Extrair textos (fase 1)

```gherkin
Cenário: SC-17 Lista de elementos com textos (fase 1)
  Dado uma imagem/frame da tela
  Quando OCR extrai textos + bounds
  Então a lista de elementos type "text" é devolvida
```

#### SC-18 — Enriquecer lista (fase 2)

```gherkin
Cenário: SC-18 Lista de elementos completa
  Dado a lista da fase 1 e o mesmo frame
  Quando visão + OCR extrai ícones, listas e imagens
  E os elementos são acrescentados na lista plana
  Então a lista completa de elementos é devolvida
```
...
```gherkin
Cenário: US-14 Lista com elementos ícone
  Dado uma imagem/frame da tela
  Quando a visão reconhece ícones
  Então a lista contém elementos type "icon"
```

#### SC-19 — Extrair ícones

```gherkin
Cenário: SC-19 Lista com elementos ícone
  Dado uma imagem/frame da tela
  Quando a visão reconhece ícones
  Então a lista contém elementos type "icon"
```
...
```gherkin
Cenário: US-15 Lista com elementos list
  Dado uma imagem/frame da tela
  Quando visão + OCR reconhecem listas/itens
  Então a lista contém elementos type "list"
```

#### SC-20 — Extrair listas

```gherkin
Cenário: SC-20 Lista com elementos list
  Dado uma imagem/frame da tela
  Quando visão + OCR reconhecem listas/itens
  Então a lista contém elementos type "list"
```
...
```gherkin
Cenário: US-16 Lista com elementos image
  Dado uma imagem/frame da tela
  Quando a visão detecta regiões de imagem/foto
  Então a lista contém elementos type "image"
```

#### SC-21 — Extrair imagens

```gherkin
Cenário: SC-21 Lista com elementos image
  Dado uma imagem/frame da tela
  Quando a visão detecta regiões de imagem/foto
  Então a lista contém elementos type "image"
```
