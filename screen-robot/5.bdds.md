...
**Piloto:** [`src/scripts/linkedin-login.js`](src/scripts/linkedin-login.js) — limpa screenshots → `resetInstance` → provision → LinkedIn → print tela inicial → árvore no console (sem digitar credenciais / sem `saveSession`).
...
Cenário: US-13 Árvore de componentes é composta em duas fases
  ...
  Quando OCR extrai textos + bounds e monta a árvore
  E em seguida visão extrai ícones/listas/imagens e completa a hierarquia
...
Cenário: SC-18 Árvore de componentes completa
  Dado a árvore da fase 1 e o mesmo frame
...
Cenário: US-14 Árvore de componentes com ícones
  Dado uma imagem/frame da tela
  Quando a visão reconhece ícones e monta a árvore
...
Cenário: US-15 Árvore de componentes com listas
  Dado uma imagem/frame da tela
  Quando visão + OCR reconhecem listas/itens e montam a árvore
...
Cenário: US-16 Árvore de componentes com imagens
  Dado uma imagem/frame da tela
  Quando a visão detecta regiões de imagem/foto e monta a árvore
