...
Cenário: US-20 Agent existente é resgatado pelo nome
  ...
  Então o retorno tem serial online e boot ok sem criar outro emulador
```
...
Cenário: SC-26 Provision anexado fica pronto
  ...
  Então o retorno tem serial online e boot ok sem novo AVD/emulador
```
...
Cenário: US-21 Tela do agent fica espelhada no scrcpy
  Dado um serial ADB online e scrcpy no PATH
  Quando openScrcpy({ serial }) é chamado
...
Cenário: SC-27 Espelhamento scrcpy aberto
  Dado serial online (EP-01) e scrcpy instalado
  Quando openScrcpy({ serial }) é executado
