...
```gherkin
Cenário: US-23 Busca por texto encontra elemento ou conjunto com score elevado
  Dado um agent com serial e tela visível
  E um texto-alvo "Sign in with Email"
  Quando findByText(serial, query) é chamado
  Então extractElements roda internamente (OCR)
  E devolve um elemento ou um conjunto de elementos vizinhos
...
```gherkin
Cenário: SC-29 Match por texto unido ou parcial com score
  Dado serial e texto-alvo
  Quando findByText encapsula extractElements e ranqueia matches
  Então o melhor resultado inclui o(s) elemento(s) e o score
  E cobre tanto match em um único text quanto união de palavras vizinhas
...
Cenário: SC-30 LinkedIn Sign in with Email (OCR partido)
  ...
  E o texto-alvo "Sign in with Email"
  Quando findByText é chamado com a fixture como frame
  Então retorna os elementos OCR ["Sign", "in", "with", "Email"]
