# Configurações para a IA — screen-robot

**Por quê:** esteira e prompts do robô de tela.  
**Umbrella / timeline:** [`../../prompts/timeline.md`](../../prompts/timeline.md) · regras: [`../../config/config-ia.md`](../../config/config-ia.md).

## Árvore

```text
README.md (visão)
├── 1.stories.md
├── 2.epics.md                  # EP → US + prioridade
├── 3.roadmap.md                # Gantt priorizado
├── 4.scenarios.md               # US → SC (arquivo único; refino pós-roadmap)
├── 5.bdds.md                    # Gherkin (arquivo único)
├── 6.prototype.md              # opcional
├── 7.tasks.md                  # Gantt TSK
├── implementation-plan/        # um .md por épico
└── sources (libs Node + scripts/linkedin-login.js)
```

Prompts novos: gravar em [`../../prompts/`](../../prompts/README.md) (próximo NNN na timeline do ConnectMax).
