...
// Ops: wipe + boot
await resetInstance({
  provision: { serial, kind: "avd" },
});
```

**Antes → depois:** handle com métodos → retorno só `{ serial, kind, provisionedAt, bootCompleted }`. Ops em módulos separados com `{ serial, … }`. API canônica: [`../src/README.md`](../src/README.md).
